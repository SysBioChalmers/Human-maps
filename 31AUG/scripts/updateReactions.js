...
const filesToReplace = readdirSync('UPDATED/').map((file)=> `UPDATED/${file}`);
        ...
        try {
            for (const [newValue, oldValue] of Object.entries(oldToNewIdMap)) {
                ...
                const optionsSecond = {
                    ...
                    from: new RegExp(`(\n| |")${oldValue}(\n| |")`, 'g'),
                    to: (match) => {
                        //console.log('Match', typeof match, '0:', match[0],'end', match[match.length-1]);
                        //console.log(match, '\n');
                        return `${match[0]}${newValue}${match[match.length-1]}`
                    },
                    countMatches: true,
                };
